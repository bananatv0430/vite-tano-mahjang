...
// .wrangler/tmp/bundle-LB7S21/checked-fetch.js
...
routes.push({
  ...
  handler: /* @__PURE__ */ __name(async ({ env, body, params }) => {
    try {
      ...
      const updatedPlayer = await db.queryOne(
        env,
        `SELECT * FROM players WHERE id = ?`,
        [id]
      );
...
async function insertLog(env, message) {
  try {
    await db.execute(
      env,
...
var logs_default = { routes: routes2 };
...
__name(formatLogDate, "formatLogDate");
routes3.push({
  method: "GET",
  path: "/",
  handler: /* @__PURE__ */ __name(async ({ env }) => {
...
routes3.push({
  ...
  handler: /* @__PURE__ */ __name(async ({ env, query }) => {
    try {
      ...
      const getJapaneseWeekday = /* @__PURE__ */ __name((dateValue) => {
        const d = /* @__PURE__ */ new Date(`${dateValue}T00:00:00`);
      ...
      for (const row of rows) {
        ...
        const dateEntry2 = dateMap.get(row.date);
        let gameEntry = dateEntry2.games.find((game) => game.gameId === row.game_id);
        if (!gameEntry) {
          gameEntry = {
            gameId: row.game_id,
          ...
          dateEntry2.games.push(gameEntry);
        }
        const playerEntry = {
        ...
        if (!dateEntry2.participants.some((player) => player.playerId === row.player_id)) {
          dateEntry2.participants.push({
            playerId: row.player_id,
            name: row.player_name,
            iconPath: row.icon_path
          });
...
routes3.push({
  ...
  handler: /* @__PURE__ */ __name(async ({ env, body }) => {
    try {
      ...
      const oka = Number(ruleRow.oka ?? 3e4);
      const gameIds = [];
      for (let i = 0; i < rounds.length; i++) {
        ...
        if (rounds[i].players) {
          for (let j = 0; j < rounds[i].players.length; j++) {
            ...
            const rankPoints = [uma1 + (oka - startPoint) * 4 / 1e3, uma2, -uma2, -uma1];
            const rankIndex = Math.max(0, Math.min(3, rank - 1));
            const basePoint = (score - oka) / 1e3;
...
routes3.push({
  method: "DELETE",
  path: "/",
  handler: /* @__PURE__ */ __name(async ({ env, query, body, request }) => {
    try {
      let date = "";
      if (query?.date) {
...
var games_default = { routes: routes3 };
...
var results_default = { routes: routes4 };
...
var rankings_default = { routes: routes5 };
...
// C:/Users/banan/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// .wrangler/tmp/bundle-LB7S21/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-LB7S21/middleware-loader.entry.ts
